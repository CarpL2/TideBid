...
import io.github.carpl2.tidebid.account.application.DepositSettlementService;
...
public class AccountDepositSettlementHandler implements AccountRocketMqTransport.InboundHandler {

    private final AccountRocketMqProperties properties;
    private final DepositSettlementService service;
    private final JdbcAccountInboxRepository inbox;
    private final EventMessageDecoder decoder;
    private final Clock clock;

    public AccountDepositSettlementHandler(
            AccountRocketMqProperties properties,
            DepositSettlementService service,
    ...
    public AccountRocketMqTransport.ConsumerBinding binding() {
        return new AccountRocketMqTransport.ConsumerBinding(
                properties.consumerGroups().depositSettlement(),
                Map.of(
                        properties.topics().auctionEvents(), DepositSettlementRequestedEvent.EVENT_TYPE,
                        properties.topics().tradeEvents(), DepositSettlementRequestedEvent.EVENT_TYPE));
    }
    ...
    public void handle(AccountRocketMqTransport.InboundMessage message) {
        ...
        if (!(envelope.payload() instanceof DepositSettlementRequestedEvent request)) {
            throw new IllegalArgumentException("message is not a deposit-settlement request");
        }
        ...
        JdbcAccountInboxRepository.InboxDecision decision = inbox.recordProcessed(
                new JdbcAccountInboxRepository.InboxEntity(
                        properties.consumerGroups().depositSettlement(), envelope.eventId().toString(),
        ...
        service.settle(envelope.eventId(), envelope.traceId(), request);
    }
    ...
    private void validateTransportMetadata(
        ...
        boolean acceptedTopic = properties.topics().auctionEvents().equals(message.topic())
                || properties.topics().tradeEvents().equals(message.topic());
        if (!acceptedTopic
                || !DepositSettlementRequestedEvent.EVENT_TYPE.equals(message.tag())
