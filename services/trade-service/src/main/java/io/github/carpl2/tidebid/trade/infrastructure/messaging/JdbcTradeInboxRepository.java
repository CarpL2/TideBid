...
public class JdbcTradeInboxRepository {

    private static final Logger LOGGER = LoggerFactory.getLogger(JdbcTradeInboxRepository.class);
    private static final String TABLE = "trade_inbox";
    ...
    public JdbcTradeInboxRepository(DataSource dataSource, MeterRegistry meters) {
    ...
    public InboxDecision recordProcessed(InboxEntity incoming) {
        ...
        try {
            ...
            if (existing.size() == 1 && existing.getFirst().sameEnvelopeAs(incoming)) {
                ...
                LOGGER.info("Duplicate inbox event absorbed: service=trade, consumerName={}, eventId={}",
    ...
    private static Counter counter(MeterRegistry meters, String outcome) {
        return meters.counter("tidebid.inbox.consume", "service", "trade", "outcome", outcome);
    }
