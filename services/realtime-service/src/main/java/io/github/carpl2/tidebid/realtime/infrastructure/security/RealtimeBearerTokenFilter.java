package io.github.carpl2.tidebid.realtime.infrastructure.security;
...
final class RealtimeBearerTokenFilter extends OncePerRequestFilter {

    private final JwtAccessTokenVerifier tokenVerifier;
    private final AuthenticationEntryPoint authenticationEntryPoint;

    RealtimeBearerTokenFilter(JwtAccessTokenVerifier tokenVerifier, AuthenticationEntryPoint authenticationEntryPoint) {
    ...
    protected void doFilterInternal(
        ...
        try {
            ...
            authenticationEntryPoint.commence(request, response, new BadCredentialsException("Access token is invalid"));
            return;
        }
    ...
    private static String bearerToken(String authorization) {
        if (!authorization.startsWith(SecurityHeaders.BEARER_PREFIX)) throw new InvalidAccessTokenException();
        String token = authorization.substring(SecurityHeaders.BEARER_PREFIX.length());
        if (token.isBlank() || token.chars().anyMatch(Character::isWhitespace)) throw new InvalidAccessTokenException();
        return token;
    }
}
