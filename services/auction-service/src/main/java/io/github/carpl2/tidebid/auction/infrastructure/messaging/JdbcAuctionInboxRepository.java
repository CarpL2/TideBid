...
public class JdbcAuctionInboxRepository {
    ...
    public JdbcAuctionInboxRepository(DataSource dataSource, MeterRegistry meters) {
    ...
    public InboxDecision recordProcessed(InboxEntity incoming) {
        ...
        try {
            ...
            if (existing.size() == 1 && existing.getFirst().sameEnvelopeAs(incoming)) {
                ...
                LOGGER.info("Duplicate inbox event absorbed: service=auction, consumerName={}, eventId={}",
    ...
    private static Counter counter(MeterRegistry meters, String outcome) {
        return meters.counter("tidebid.inbox.consume", "service", "auction", "outcome", outcome);
    }
