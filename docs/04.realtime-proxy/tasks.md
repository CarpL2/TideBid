...
- [x] 更新阶段 03 的“无实时推送”文案，保留 MySQL 最终裁决说明。（2026-09-25：阶段 03 历史文档保留其阶段边界，README 与阶段 04 文档已说明 Realtime 只提升体验，MySQL 仍是最终事实源。）
...
- [ ] 使用两个浏览器用户演示代理反击、反狙击延时、断线恢复和最终订单。（2026-09-25：Realtime 全停期间完整 `-ReliableTrade` 通过；恢复后 `-RealtimeProxyTradeDemo` 继续通过 Snapshot、代理、关拍和订单入口。新增 `scripts/browser-smoke.ps1` 并通过 Chrome headless DOM 检查，当前仅待真实浏览器 Console/布局/Network 验收。）
- [x] 运行根目录 `mvn clean verify`，所有模块和真实 MySQL 核心测试通过。（2026-09-25：停止应用进程后执行 `mvn clean verify`，11 个 Reactor 模块 BUILD SUCCESS；新增重复 eventId/无订阅缓存测试通过，真实 MySQL 核心链路另由 `-ReliableTrade`、`-RealtimeBrokerRecovery` 等整栈 smoke 覆盖。）
