...
- [ ] 使用两个浏览器用户演示代理反击、反狙击延时、断线恢复和最终订单。（2026-09-25：`smoke.ps1 -RealtimeProxyTradeDemo` 已补齐 B 提高代理、A/B 同 eventId、MySQL 历史、双端 `AUCTION_CLOSED` 和 `PENDING_PAYMENT` 订单入口；当前仍待真实浏览器页面及支付后订单/结算。）
