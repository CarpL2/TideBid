...
- [ ] 使用两个浏览器用户演示代理反击、反狙击延时、断线恢复和最终订单。（2026-09-25：`smoke.ps1 -RealtimeProxyPaymentDemo` 已补齐 B 提高代理、A/B 同 eventId、MySQL 历史、双端 `AUCTION_CLOSED`、支付幂等和 `PAID/COMPLETED` 结算；Chrome headless DOM 已确认前端挂载，当前仍待真实浏览器 Console/布局/Network 验收。）
